...
/// 施工安全质量典型案例
class CsExampleProvider with ChangeNotifier {
  ...
  Future getData({int status = 0, int pid, isReset: false}) async {
    ...
    notifyListeners();
    await handleData((await searchCsExample(payload)));
  }
  ...
  Future<bool> handleCreate() async {
    ...
    BaseResp res = await createCsExample(payload);
    if (res.status == '200') {
      Global.formRecord = {};
      return true;
    }
  ...
  Future<bool> handleRemove() async {
    BaseResp res = await removeCsExample(Global.formRecord['id']);
    if (res.status == '200') {
      listData = listData
