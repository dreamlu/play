...
class BottomOperation extends StatelessWidget {
  ...
  handleCreate(BuildContext context, bool isRemove) async {
    if (isRemove) {
      ...
      if (isConfirm) {
        ...
        await context.read<CsExampleProvider>().handleCreate();
        await context.read<CsExampleProvider>().getData(isReset: true, pid: 0);
      }
    }
  }

  handleRemove(BuildContext context, bool isRemove) async {
    if (isRemove) {
